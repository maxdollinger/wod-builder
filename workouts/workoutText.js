...
const createAmrapText = workout =>
     `Amrap ${timeString(workout.time[0])}\n`;

const createEmomText = workout =>
     `Every ${timeString(workout.time[0])} for ${timeString(workout.time[0] * workout.sections[0])}\n`;

const createAltEmomText = workout =>
     `Alternate every ${timeString(workout.time[0])} for ${timeString(workout.time[0] * workout.sections[0])} between\n`;

const createRftText = workout => `${workout.sections[0]} Rounds of\n`;

const createFtText = workout => `For time do\n`;

const createTabataText = workout =>
     `Tabata ${workout.sections[0]}x${timeString(workout.time[0])}/${timeString(workout.rest[0])}\n`;
